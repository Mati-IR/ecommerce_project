...
def save_upload_file_tmp(upload_file: None, raw_data_file = None) -> Path:
    try:
        ...
        if upload_file:
            ...
            raw_data_file.close()
        logger.info(f"save_upload_file_tmp: final_temp_file: {final_temp_file}, extension: {extension}")
...
def handle_upload_image_file(thumbnail, upload_file: None, raw_data_file = None):
    try:
        ...
        logger.info(f"handle_upload_image_file: file_extension: {file_extension}, file_extension[1:]: {file_extension[1:]}, IMAGE_AllOWED_FILE_FORMAT: {os.environ.get('IMAGE_AllOWED_FILE_FORMAT', '').split(',')}")
        ...
        if file_extension[1:] in os.environ.get('IMAGE_AllOWED_FILE_FORMAT', '').split(','):
            ...
            logger.info(f"handle_upload_image_file: imagePaths: {imagePaths}")
            return imagePaths
        else:
            logger.error(f"handle_upload_image_file: {file_extension[1:]} not supported, supported file types: {os.environ.get('IMAGE_AllOWED_FILE_FORMAT', '').split(',')}")
